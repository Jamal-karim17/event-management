...
export class EventService {
  ...
  getEventById(id: number | string): Observable<Event> {
    const eventId = typeof id === 'string' ? +id : id; // Convert id to number if it's a string
    return this.http.get<Event>(`${this.baseUrl}/${eventId}`);
  }

  // Add a new event, auto-generate and use a number ID
  addEvent(event: Event): Observable<Event> {
    // Ensure the ID is a sequential number and it remains a number
    this.lastUsedId += 1;  // Increment the last used ID
    event.id = this.lastUsedId; // Use the number ID
  ...
  updateEvent(event: Event): Observable<Event> {
    const eventId = typeof event.id === 'string' ? +event.id : event.id; // Convert id to number if it's a string
